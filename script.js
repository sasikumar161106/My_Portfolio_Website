...
python main.py</pre>
    ...
    /* ==========================================================================
       HERO PROXIMITY REVEAL — Global cursor tracking (GSAP)
       ========================================================================== */
    (function initProximityReveal() {
        ...
        if (!container || !suitLayer) return;

        // Guard: make sure GSAP loaded
        if (typeof gsap === 'undefined') {
            console.warn('[Proximity Reveal] GSAP not found — skipping animation.');
            return;
        }
        ...
        // Function to calculate distance from point (px, py) to a rectangle (rect)
        function getDistanceFromRect(px, py, rect) {
            // Find the closest point on the rect to the cursor
            const closestX = Math.max(rect.left, Math.min(px, rect.right));
            const closestY = Math.max(rect.top, Math.min(py, rect.bottom));
            
            // Calculate distance from cursor to that closest point
        ...
        function handleMouseMove(e) {
            ...
            // Distance is 0 if cursor is INSIDE the rect, >0 if outside
            const distance = getDistanceFromRect(e.clientX, e.clientY, rect);
            
            const shouldReveal = distance <= PROXIMITY_THRESHOLD;
            ...
            if (shouldReveal && !isRevealed) {
                ...
                isRevealed = false;
                // Fade out (leave) — cinematic dissolve
                gsap.to(suitLayer, {
                    opacity: 0,
                    duration: 1.5,
                    ease: 'power3.inOut',
        ...
        // We track on the window to catch proximity from anywhere
